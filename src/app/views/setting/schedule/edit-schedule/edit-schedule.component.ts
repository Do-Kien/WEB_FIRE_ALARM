...
export class EditScheduleComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.getScheduleInfo(this.currentSchedule);
  ...
  checkCurrentData() {
    this.currentSchedule = this.authObj.getCurrentSchedule();
  ...
  getScheduleInfo(scheduleId: string){
    console.log("IDDDDDDDDDDDDDDDD" + scheduleId)
    if((scheduleId != undefined) && (scheduleId != null)){
      this.obj.getRuleInfo(scheduleId).subscribe({
        next: (res: any) =>{
          console.log("GETTTTTT SCHEDULE")
          console.log(res)
          ...
          this.filterGreenhouse = res.greenhouseId;
          this.filterZoneAll = res.zoneId;
          this.convertNumberToDay(res.loop);

          for(let i = 0; i < res.timer.length; i++){
            this.addValueToTimeForm(res.timer[i].timeStart, res.timer[i].timeStop);
          }

          this.items.controls.shift();
          console.log(this.items.controls)
          this.target = res.relay;
        },
        error: (e) => {
          console.error(e); 
        }
  ...
  addSchedule(){
    ...
    console.log("DAYYYYYY" + this.selectedDayItems);
    ...
    if((checkTime == true) && (this.formSchedule.valid) && (this.formSchedule.value.id != null)){
      console.log(this.formSchedule.value)
      this.obj.updateRule(this.formSchedule.value.id, this.formSchedule.value).subscribe({
        next: (res: any) =>{
          console.log(res);
          this.alerts.showAlerts(titleType.schedule, "Cập nhật lịch thành công", toastType.success);
  ...
  addValueToTimeForm(timeStart: string, timeStop: string){
    // console.log(this.form.get("time")?.value)
    this.items = this.form.get("timer") as FormArray;
